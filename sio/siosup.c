...
static char RCSid[] = "$Id: siosup.c,v 8.4 1993/03/17 07:35:24 panos Exp $" ;
	...
	struct stat *stp ;
{
	...
	if ( try_memory_mapping( fd, idp, stp ) == FAILURE )
	{
		 ...
		 */
#ifdef VMS
		unsigned buf_size = (unsigned) SIO_BUFFER_SIZE;
#else
...
#ifdef VMS
	buf_size = (unsigned) SIO_BUFFER_SIZE ;
#else
	buf_size = (unsigned)
						( stp->st_blksize ? stp->st_blksize : SIO_BUFFER_SIZE ) ;
#endif
...
#ifdef MULTINET
		cc = socket_write( fd, odp->start, b_in_buffer ) ;
#else
	...
	int fd ;
{
	...
	{
#ifdef MULTINET
		cc = socket_read( fd, idp->buf, (int) idp->buffer_size ) ;
#else
	...
	int fd ;
{
	...
	{
#ifdef MULTINET
		cc = socket_read( fd, idp->end, b_left ) ;
#else
