...
DIR *
opendirx(const char *dirname, char *pattern)
{
	XDIR *dirp;
	char *pathname;
	HANDLE dirHandle;
...
void
joinpath(char *str, char *dir, char *file)
{
	char c;
	struct passwd *pw;
...
int
make_post_cmd(char *cmd, char *name)
{
	char *p;
...
void
GetConsoleCursorPosition(HANDLE hCon, COORD * pCoord)
{
 ...
 */
int
win32rename(char *src, char *dest)
{
...
#ifdef DEBUG
void
DebugAssertDialog(LPSTR szMessage, DWORD dwLine, LPSTR szFile)
{
